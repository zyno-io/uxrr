...
describe('AppResolverService', () => {
    describe('resolveByOrigin', () => {
        it('maps known origin to appKey', async () => {
            ...
            assert.equal(result?.appKey, 'app-1');
        });
        ...
        it('handles multiple origins per app', async () => {
            const { db } = createMocks([
                { id: 'uuid-1', appKey: 'app-1', origins: ['https://a.com', 'https://b.com'], apiKey: undefined, isActive: true }
            ]);
            const resolver = new AppResolverService(db, makeLogger());
    ...
    describe('resolveByApiKey', () => {
        it('maps known API key hash to appKey', async () => {
            const rawKey = 'test-api-key-123';
            const keyHash = createHash('sha256').update(rawKey).digest('hex');
            const { db } = createMocks([{ id: 'uuid-2', appKey: 'app-2', origins: [], apiKey: keyHash, isActive: true }]);
            const resolver = new AppResolverService(db, makeLogger());

            const result = await resolver.resolveByApiKey(rawKey);
            assert.equal(result?.appKey, 'app-2');
        });
    ...
    describe('cache TTL', () => {
        ...
        it('re-queries after cache invalidation', async () => {
            const apps = [{ id: 'uuid-1', appKey: 'app-1', origins: ['https://example.com'], apiKey: undefined, isActive: true }];
            ...
            assert.equal(next?.appKey, 'app-1');
    ...
    describe('getAllowedOrigins', () => {
        it('returns all registered origins', async () => {
            const { db } = createMocks([
                { id: 'uuid-1', appKey: 'app-1', origins: ['https://a.com', 'https://b.com'], apiKey: undefined, isActive: true },
                { id: 'uuid-2', appKey: 'app-2', origins: ['https://c.com'], apiKey: undefined, isActive: true }
            ]);
            const resolver = new AppResolverService(db, makeLogger());
