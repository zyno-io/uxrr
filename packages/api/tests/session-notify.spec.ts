...
describe('SessionNotifyService — allowedAppKeys enforcement', () => {
    it('sendToLocalWatchers skips watchers with non-matching allowedAppKeys', () => {
        const svc = new SessionNotifyService(makeLogger(), makeRedis(), makePresence(), makeAppResolver());
    ...
    it('sendToLocalWatchers sends to watchers with matching allowedAppKeys', () => {
        const svc = new SessionNotifyService(makeLogger(), makeRedis(), makePresence(), makeAppResolver());
    ...
    it('sendToLocalWatchers sends to watchers with no allowedAppKeys restriction', () => {
        const svc = new SessionNotifyService(makeLogger(), makeRedis(), makePresence(), makeAppResolver());
    ...
    it('notifySessionUpdated also enforces allowedAppKeys', () => {
        const svc = new SessionNotifyService(makeLogger(), makeRedis(), makePresence(), makeAppResolver());
    ...
    it('Redis notification enforces allowedAppKeys on remote watchers', () => {
        ...
        const svc = new SessionNotifyService(makeLogger(), redis, makePresence('pod-1'), makeAppResolver());
    ...
    it('stale checker enforces allowedAppKeys', () => {
...
describe('SessionNotifyService — watcher filter enforcement', () => {
    it('set_filters with disallowed appKey is silently dropped', () => {
        const svc = new SessionNotifyService(makeLogger(), makeRedis(), makePresence(), makeAppResolver());

        const { ws, sent } = makeWs();
        svc.addWatcher(ws, { appKey: 'app-1' }, ['app-1']); // allowed app-1 only
    ...
    it('set_filters auto-sets appKey when only one allowed app', () => {
        const svc = new SessionNotifyService(makeLogger(), makeRedis(), makePresence(), makeAppResolver());

        const { ws, sent } = makeWs();
        svc.addWatcher(ws, {}, ['app-1']); // allowed app-1 only

        // Set filters without specifying appKey
        ws.emit(
            'message',
            Buffer.from(
                JSON.stringify({
