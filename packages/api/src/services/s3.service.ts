...
export class S3Service {
    ...
    async putEvents(appId: string, sessionId: string, chunkIndex: number, data: unknown): Promise<void> {
        const key = this.buildKey(appId, sessionId, 'events', chunkIndex);
        const compressed = await gzipAsync(JSON.stringify(data));
        await this.putCompressed(key, compressed);
    }

    async putEventsCompressed(appId: string, sessionId: string, chunkIndex: number, compressedData: Buffer): Promise<void> {
        const key = this.buildKey(appId, sessionId, 'events', chunkIndex);
        await this.putCompressed(key, compressedData);
    }

    async getEvents(appId: string, sessionId: string, startChunk?: number, endChunk?: number): Promise<unknown[]> {
        if (startChunk !== undefined && endChunk !== undefined) {
            return this.getChunksByRange(appId, sessionId, 'events', startChunk, endChunk);
        }
        return this.getChunks(appId, sessionId, 'events');
    }

    async putChat(appId: string, sessionId: string, messages: unknown[]): Promise<void> {
        const key = `${appId}/chat/${sessionId}.json`;
        await this.put(key, messages);
    }

    async getChat(appId: string, sessionId: string): Promise<unknown[]> {
        const key = `${appId}/chat/${sessionId}.json`;
        try {
    ...
    async deleteSessionEvents(appId: string, sessionId: string): Promise<void> {
        await this.deleteByPrefix(`${appId}/${sessionId}/events/`);
    }

    async deleteSessionChat(appId: string, sessionId: string): Promise<void> {
        const key = `${appId}/chat/${sessionId}.json`;
        try {
    ...
    private buildKey(appId: string, sessionId: string, type: string, chunkIndex: number): string {
        const chunk = String(chunkIndex).padStart(6, '0');
        return `${appId}/${sessionId}/${type}/${chunk}.json`;
    }
    ...
    private async getChunksByRange(appId: string, sessionId: string, type: string, startChunk: number, endChunk: number): Promise<unknown[]> {
        ...
        const fetched: unknown[][] = Array.from<unknown[]>({ length: keys.length });
    ...
    private async getChunks(appId: string, sessionId: string, type: string): Promise<unknown[]> {
        const prefix = `${appId}/${sessionId}/${type}/`;
