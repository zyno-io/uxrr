...
export class IngestService {
    ...
    async ingestData(appUuid: string, appKey: string, sessionId: string, payload: IngestDataPayload, ipAddress?: string): Promise<void> {
        ...
        if (hasEvents) {
            await this.enqueueEvents(appUuid, appKey, sessionId, payload.events!);
    ...
    async forwardOtlp(
        ...
        if (appKey && !body.includes(`{"key":"service.name","value":{"stringValue":"${appKey}"}}`)) {
            return { status: 403, contentType: 'text/plain', body: 'service.name does not match appKey' };
        }
    ...
    private async enqueueEvents(appId: string, appKey: string, sessionId: string, events: RrwebEvent[]): Promise<void> {
        let buffer = this.eventBuffers.get(sessionId);
        if (!buffer) {
            buffer = { appId, appKey, events: [], approxBytes: 0, flushRequested: false };
