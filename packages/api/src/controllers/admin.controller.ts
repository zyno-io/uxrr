...
export class AdminController {
    ...
    async createApp(body: HttpBody<CreateAppBody>): Promise<AppResponse> {
        ...
        if (!appKey) {
            throw new HttpBadRequestError('appKey is required');
        }

        const existing = await this.db.query(AppEntity).filter({ appKey }).findOneOrUndefined();
        if (existing) {
            throw new HttpUserError(`App key "${appKey}" is already in use`);
        }
        ...
        this.logger.info(`App ${app.appKey} created: ${app.name}`);
    ...
    async updateApp(appKey: string, body: HttpBody<UpdateAppBody>): Promise<AppResponse> {
        ...
        await this.db.persist(app);
        this.appResolver.invalidateCache();
        this.logger.info(`App ${appKey} updated`);
        return this.toAppResponse(app);
    }
    ...
    async deactivateApp(appKey: string): Promise<{ ok: boolean }> {
        ...
        await this.db.persist(app);
        this.appResolver.invalidateCache();
        this.logger.info(`App ${appKey} deactivated`);
        return { ok: true };
    }
