...
export class ShareController {
    ...
    async getSession(token: string): Promise<IShareSession> {
        ...
        const appId = this.resolveAppKey(session);
        return { ...session, appId, allUserIds: userIdMap.get(session.id) ?? [], isLive } as IShareSession;
    }

    @http.GET(':token/events')
    async getSessionEvents(token: string, query: HttpQueries<{ startChunk?: number; endChunk?: number }>): Promise<IRrwebEvent[]> {
        const session = await this.resolveSession(token);
        return this.s3Svc.getEvents(session.appId, session.id, query.startChunk, query.endChunk) as Promise<IRrwebEvent[]>;
    }
