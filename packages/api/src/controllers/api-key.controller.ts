...
export class ApiKeyController {
    ...
    async createKey(body: HttpBody<CreateApiKeyBody>): Promise<ApiKeyResponse & { key: string }> {
        ...
        if (body.appKeys.length > 0) {
            await this.validateAppKeys(body.appKeys);
        }
        const { key, rawKey } = await this.apiKeySvc.create(body.name.trim(), body.scope, body.appKeys);
    ...
    async updateKey(id: string, body: HttpBody<UpdateApiKeyBody>): Promise<ApiKeyResponse> {
        ...
        if (body.appKeys !== undefined && body.appKeys.length > 0) {
            await this.validateAppKeys(body.appKeys);
        }
    ...
    private async validateAppKeys(appKeys: string[]): Promise<void> {
        const apps = await this.db
            .query(AppEntity)
            .filter({ id: { $in: appKeys } })
            .find();
        const foundIds = new Set(apps.map(a => a.id));
        const invalid = appKeys.filter(id => !foundIds.has(id));
        if (invalid.length > 0) {
            throw new HttpBadRequestError(`Unknown app keys: ${invalid.join(', ')}`);
        }
    }
