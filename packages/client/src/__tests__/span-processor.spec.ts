...
describe('IdentitySpanProcessor', () => {
    ...
    it('skips identity attributes when includeIdentityAttributes is false', () => {
        const delegate = makeDelegate();
        const identity = makeIdentity();
        const processor = new IdentitySpanProcessor(delegate, identity, 'sess-1', undefined, false);

        const span = makeSpan();
        processor.onStart(span, {} as unknown as Context);

        expect(identity.toSpanAttributes).not.toHaveBeenCalled();
        expect(span.setAttribute).not.toHaveBeenCalledWith('uxrr.device_id', expect.anything());
        expect(span.setAttribute).not.toHaveBeenCalledWith('uxrr.user_id', expect.anything());
        // sid should still be set
        expect(span.setAttribute).toHaveBeenCalledWith('uxrr.sid', 'sess-1');
    });
