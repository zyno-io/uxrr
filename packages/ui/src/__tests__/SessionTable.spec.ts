...
describe('SessionTable', () => {
    ...
    it('renders session rows', () => {
        ...
        const rows = wrapper.findAll('tbody tr.row-clickable');
    ...
    it('displays dash for missing version/environment', () => {
        ...
        const row = wrapper.find('tbody tr.row-clickable');
        const cells = row.findAll('td');
        expect(cells[6]!.text()).toBe('-'); // version
        expect(cells[7]!.text()).toBe('-'); // environment
    });

    it('renders sentinel row when hasMore is true and sessions exist', () => {
        const session = makeSession();
        const wrapper = mount(SessionTable, {
            global,
            props: { sessions: [session], loading: false, error: null, hasMore: true }
        });
        expect(wrapper.find('.cell-loading-more').exists()).toBe(true);
    });

    it('hides sentinel row when hasMore is false', () => {
        const session = makeSession();
        const wrapper = mount(SessionTable, {
            global,
            props: { sessions: [session], loading: false, error: null, hasMore: false }
        });
        expect(wrapper.find('.cell-loading-more').exists()).toBe(false);
    });

    it('shows loading text in sentinel when loadingMore is true', () => {
        const session = makeSession();
        const wrapper = mount(SessionTable, {
            global,
            props: { sessions: [session], loading: false, error: null, hasMore: true, loadingMore: true }
        });
        expect(wrapper.find('.cell-loading-more').text()).toContain('Loading more...');
    });

    it('does not render sentinel when sessions are empty', () => {
        const wrapper = mount(SessionTable, {
            global,
            props: { sessions: [], loading: false, error: null, hasMore: true }
        });
        expect(wrapper.find('.cell-loading-more').exists()).toBe(false);
    });
});
