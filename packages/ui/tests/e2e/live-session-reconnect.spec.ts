...
async function ensureSdkAppOrigin(origin: string): Promise<void> {
    ...
    if (!listResponse.ok) throw new Error(`Failed to list apps (${listResponse.status})`);
    const apps = (await listResponse.json()) as Array<{ appKey: string; origins: string[]; isActive: boolean }>;
    const existing = apps.find(app => app.appKey === SDK_APP_KEY);
    if (!existing) {
        const res = await fetch(`${API_BASE_URL}/v1/admin/apps`, {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ appKey: SDK_APP_KEY, name: 'SDK Test App', origins: [origin] })
        });
        if (!res.ok) throw new Error(`Failed to create app (${res.status}): ${await res.text()}`);
        return;
    }
    const nextOrigins = Array.from(new Set([...(existing.origins ?? []), origin]));
    if (existing.isActive && nextOrigins.length === existing.origins.length) return;
    const res = await fetch(`${API_BASE_URL}/v1/admin/apps/${SDK_APP_KEY}`, {
        method: 'PATCH',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ origins: nextOrigins, isActive: true })
    });
    if (!res.ok) throw new Error(`Failed to patch app (${res.status}): ${await res.text()}`);
}
