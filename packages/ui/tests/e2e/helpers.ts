...
export function mockSessionListRoutes(page: Page, sessions: ISession[]) {
    return Promise.all([
        ...
        json(page, '**/v1/sessions/autocomplete/appKeys*', ['my-app', 'other-app']),
...
export function startStaticServer(opts: StaticServerOptions): { server: Server; baseUrl: string } {
    const server = createServer((req, res) => {
        ...
        // Serve chunk files from the client SDK dist directory (code-split dynamic imports)
        const sdkDistDir = dirname(opts.clientSdkPath);
        const chunkPath = join(sdkDistDir, url);
        if (existsSync(chunkPath) && extname(chunkPath) === '.js') {
